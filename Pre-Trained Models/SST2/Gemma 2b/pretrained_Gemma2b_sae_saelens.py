...
try:
    ...
    # OPTION B: Manual login (paste your hf_... token when prompted)
    login()
...
# ======================================================================
# 5. STEP 2: Layer-wise Analysis (Probes + Gemma Scope SAEs)
# ======================================================================
print("\n" + "="*70)
...
TOP_K = 10  # top-k frequent features
...
for layer_num in range(model.cfg.n_layers):
    ...
    print("Top 5 Most Activating Features (POSITIVE) - by avg activation:")
    for rank, (feat_idx, avg_act) in enumerate(top5_pos_by_activation, 1):
        ...
        print(
            f"  {rank}. Feature {feat_idx}: avg_act={avg_act:.4f}, "
            f"freq={freq} ({pct:.1f}%) | {status}"
        )

    print("\nTop 5 Most Activating Features (NEGATIVE) - by avg activation:")
    ...
    print(
        f"\nCommon features among top-{TOP_K} POS and NEG: "
        f"{len(topk_common_ids)}"
    )
    if len(topk_common_ids) > 0:
        print("  IDs (first up to 10):", sorted(list(topk_common_ids))[:10])

    # save per-layer detailed file
    with open(f"/kaggle/working/results/layer_{layer_num}_complete_analysis.txt", "w") as f:
        f.write("="*70 + "\n")
        f.write(f"LAYER {layer_num} - COMPLETE ANALYSIS (Gemma Scope)\n")
        f.write("="*70 + "\n\n")
...
with open("/kaggle/working/results/final_summary.txt", "w") as f:
    f.write("="*70 + "\n")
    f.write("FINAL SUMMARY - GEMMA SCOPE LAYER-WISE ANALYSIS\n")
    f.write("="*70 + "\n\n")
